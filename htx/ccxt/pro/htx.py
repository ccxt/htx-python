...
class htx(htxAsync):
    ...
    def get_order_channel_and_message_hash(self, type: object, subType: object, market: Market = None, params={}):
        messageHash = None
        channel = None
    ...
    def get_v5_linear_channel_and_message_hash(self, topic: object, market: Market = None, params={}):
    ...
    def parse_ws_order(self, order: object, market: Market = None):
        ...
        filled = self.safe_string_2(order, 'execAmt', 'trade_volume')
        ...
        cost = self.safe_string_2(order, 'orderValue', 'trade_turnover')
    ...
    def handle_positions(self, client: object, message: object):
        ...
        for i in range(0, len(marginModes)):
            ...
            for j in range(0, len(messageHashes)):
                ...
                positions = self.filter_by_array(marginModePositions, 'symbol', symbols, False)
            ...
            if topic == 'account':
                ...
                for i in range(0, detailsLength):
                    ...
                    if code is None:
                        continue
                    account = self.account()
                    account['free'] = self.safe_string(detail, 'withdraw_available')
                    account['total'] = self.safe_string(detail, 'equity')
                    self.balance[code] = account
                self.balance = self.safe_balance(self.balance)
                client.resolve(self.balance, 'account')
                return
            dataLength = len(data)
            if dataLength == 0:
                return
    ...
    def handle_my_trade(self, client: Client, message: object, extendParams={}):
        #
        # spot
        #
        #     {
        ...
        if messageHash is not None:
            ...
            if data is not None:
                ...
                if (messageHash == 'trade') and (contractCode is not None):
    ...
    def parse_ws_trade(self, trade: object, market: Market = None):
        ...
        amount = self.safe_string_2(trade, 'tradeVolume', 'trade_volume')
    ...
    def get_url_by_market_type(self, type: object, isLinear=True, isPrivate=False, isFeed=False, isV5=False):
        ...
        if type == 'spot':
            ...
            if isPrivate:
                if isV5 and isLinear:
                    url = self.safe_string(subTypeUrl, 'privateV5', subTypeUrl['private'])
                else:
    ...
    async def subscribe_public(self, url: object, symbol: object, messageHash: object, method: object = None, params={}):
        requestId = self.request_id()
        request = {
            'sub': messageHash,
            'id': requestId,
    ...
    async def subscribe_private(self, channel: object, messageHash: object, type: object, subtype: object, params: object = {}, subscriptionParams={}):
        requestId = self.request_id()
        subscription = {
            'id': requestId,
            'messageHash': messageHash,
